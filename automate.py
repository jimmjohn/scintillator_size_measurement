...
# perform edge detection, then perform a dilation + erosion to
# close gaps in between object edges
edged2 = cv2.Canny(thresh2, 10, 100,apertureSize = 3)
...
for c in cnts2:
	# if the contour is not sufficiently large, ignore it
	if (cv2.contourArea(c) < 70000) or (cv2.contourArea(c) > 200000):
		continue
	# approximate the contour
...
cv2.putText(calibrator, "{:.1f}mm".format(heightinmm),
	(int(x-100), int(y+(h/2)+15)), cv2.FONT_HERSHEY_SIMPLEX,
	0.65, (255, 255, 255), 2)
...
edged3 = cv2.dilate(edged3, kernel3, iterations=1)
#edged = cv2.erode(edged, kernel, iterations=1)
